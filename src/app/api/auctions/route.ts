...
export async function GET(request: NextRequest) {
  ...
  if (live) {
    const auctions = await prisma.auction.findMany({ where: { status: 'LIVE' }, include: { items: true } });

    // compute currentMin for each item
    for (const a of auctions) {
      for (const it of a.items) {
        const minBid = await prisma.bid.findFirst({ where: { itemId: it.id }, orderBy: { bidValue: 'asc' } });
...
export async function POST(request: NextRequest) {
  ...
  const withItems = await prisma.auction.findUnique({ where: { id: auction.id }, include: { items: true } });
